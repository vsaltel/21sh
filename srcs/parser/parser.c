/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parser.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: frossiny <frossiny@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/21 15:17:59 by frossiny          #+#    #+#             */
/*   Updated: 2019/04/08 15:32:05 by frossiny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "shell.h"

#include <fcntl.h>
static void	disp_tree(t_anode *ast)
{
	//ft_printf("--- AST Tree ---\n");



	//ft_printf("----------------\n");
}

static t_token *getCmdEnd(t_token *tokens)
{
	while (tokens && is_word_token(tokens))
		tokens = tokens->next;
	return (tokens);
}

t_cmd *create_cmd(t_token *exe, char **args, int argc)
{
	t_cmd *cmd;

	if (!(cmd = (t_cmd *)malloc(sizeof(t_cmd))))
		return (NULL);
	cmd->exe = exe;
	cmd->argc = argc;
	cmd->args = args;
	return (cmd);
}

static int	parse_tree(t_token *tokens, t_anode **ast, t_env **env)
{
	t_anode	*tree;
	t_anode	*tmp;
	char	**argv;
	int		argc;
	t_cmd	*cmd;
	t_token	*curr;
	
	//TODO REPLACE VARS
	tree = NULL;
	curr = tokens;
	while (tokens)
	{
		ft_printf("Token: %s\n", tokens->content);
		if (tokens->type == TOKEN_PIPE)
		{
			if (!tree)
				return (0);
			ft_printf("New PIPE Node on %s\n", tree->ope ? tree->ope->content : tree->cmd->exe->content);
			tmp = create_node(tokens, NULL);
			tmp->left = tree;
			argc = build_args(&argv, tokens->next);
			ft_printf("New CMD node on %s: %s\n", tmp->ope->content, tokens->next->content);
			tmp->right = create_node(NULL, create_cmd(tokens->next, argv, argc));
			tree = tmp;
			tokens = getCmdEnd(tokens->next);
			curr = tokens;
		}
		else if (tokens->type == TOKEN_AND || tokens->type == TOKEN_OR)
		{
			if (!tree)
				return (0);
			ft_printf("New CONDITION Node on %s\n", tree->ope ? tree->ope->content : tree->cmd->exe->content);
			tmp = create_node(tokens, NULL);
			tmp->left = tree;
			argc = build_args(&argv, tokens->next);
			ft_printf("New CMD node on %s: %s\n", tmp->ope->content, tokens->next->content);
			tmp->right = create_node(NULL, create_cmd(tokens->next, argv, argc));
			tree = tmp;
			tokens = getCmdEnd(tokens->next);
			curr = tokens;
		}
		else if (is_word_token(tokens))
		{
			argc = build_args(&argv, curr);
			cmd = create_cmd(curr, argv, argc);
			if (!tree)
			{
				ft_printf("First node: %s\n", cmd->exe->content);
				tree = create_node(NULL, cmd);
			}
			else
			{
				ft_printf("New CMD node on %s\n", tree->ope ? tree->ope->content : tree->cmd->exe->content);
				tmp = create_node(NULL, cmd);
				tmp->left = tree;
				tree = tmp;
				
			}
			tokens = getCmdEnd(tokens);
			curr = tokens;
		}
		else
			tokens = tokens->next;
	}
	disp_tree(tree);
	*ast = tree;
	return (1);
}

int			parse(t_lexer *lexer, t_env **env)
{
	t_token	*tokens;
	t_anode	*ast;

	if (lexer->tokens && lexer->tokens->content && !ft_strcmp(lexer->tokens->content, "exit")) //TEMP
		exit(120);

	return (parse_tree(lexer->tokens, &ast, env));
}
